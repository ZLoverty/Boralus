...
def compute_joint_anchors(
    ...
    if lock_xy_to_fallback:
        alpha = float(np.clip(xy_blend, 0.0, 1.0))
        fallback_xy = np.asarray(fallback_center, dtype=float)[:2]
        head_center[:2] = (1.0 - alpha) * head_center[:2] + alpha * fallback_xy
        body_center[:2] = (1.0 - alpha) * body_center[:2] + alpha * fallback_xy
...
def make_joint(
    ...
    def _xy_center_from_bounds(mesh: trimesh.Trimesh) -> np.ndarray:
        mins = mesh.vertices[:, :2].min(axis=0)
        maxs = mesh.vertices[:, :2].max(axis=0)
        return (mins + maxs) * 0.5

    def _place_tool_base(
        tool_mesh: trimesh.Trimesh,
        anchor_center: Sequence[float],
        z_offset: float = 0.0,
    ) -> None:
        anchor = np.asarray(anchor_center, dtype=float)
        target_xy = anchor[:2]
        target_z = float(anchor[2] + z_offset)

        current_xy = _xy_center_from_bounds(tool_mesh)
        current_min_z = float(tool_mesh.vertices[:, 2].min())
        tool_mesh.apply_translation(
            np.array(
                [
                    target_xy[0] - current_xy[0],
                    target_xy[1] - current_xy[1],
                    target_z - current_min_z,
                ],
                dtype=float,
            )
        )
    ...
    # Female socket: tool base shares the head interface plane.
    _place_tool_base(female_tool, head_anchor_center, z_offset=0.0)
    head_slot = map_color(head_part, head_part.difference(female_tool, engine=boolean_engine))

    # Male peg: tool base sits on top of the body interface plane.
    male_tool.apply_scale(scale)
    _place_tool_base(male_tool, body_anchor_center, z_offset=0.0)
